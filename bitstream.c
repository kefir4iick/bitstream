...
static int ReadBit(BitStream *bs, int *bit) {
    ...
    *bit = (bs->buffer >> (7 - bs->bit_pos)) & 1;
    bs->bit_pos = (bs->bit_pos + 1) & 7;
    return 1;
}
...
void WriteUInt64(BitStream *bs, uint64_t x, size_t bit_len) //dop
{
    uint8_t buf[8];
    ...
    WriteBitSequence(bs, buf, bit_len);
}

uint64_t ReadUInt64(BitStream *bs, size_t bit_len)
{
    uint8_t buf[8] = {0};
    ReadBitSequence(bs, buf, bit_len);
